/**
 * Gemini Live API voice session for Web.
 *
 * Implements VoiceSession using Google's Gemini Multimodal Live API
 * over WebSocket with AudioWorklet for audio capture/playback.
 *
 * Architecture:
 *   Browser mic → AudioWorklet → PCM 16kHz → base64 → WebSocket → Gemini
 *   Gemini → WebSocket → base64 PCM 24kHz → AudioWorklet → Speaker
 */
...
class GeminiVoiceSessionImpl implements VoiceSession {
        ...
        try {
                        ...
                        functionDeclarations: [
                            {
                                ...
                                parameters: {
                                    ...
                                    properties: {
                                        message: {
                                            type: Type.STRING,
                                            description: 'The full message to send to the coding agent',
                                        },
                                    },
                                    required: ['message'],
                                },
                            },
                            {
                                ...
                                description: 'Approve or deny a pending permission request from the coding agent. Use when the user says "yes", "allow", "go ahead" (allow) or "no", "deny", "cancel" (deny).',
                                parameters: {
                                    type: Type.OBJECT,
                                    properties: {
                                        decision: {
                                            type: Type.STRING,
                                            description: 'The decision: "allow" or "deny"',
                                        },
                                    },
                                    required: ['decision'],
                                },
            ...
            if (config.initialContext) {
                this.session.sendClientContent({
                    turns: [{
                        role: 'user',
                        parts: [{ text: `[CONTEXT UPDATE] ${config.initialContext}` }],
                    }],
                    turnComplete: true,
                });
            }
    ...
    private handleServerMessage(message: LiveServerMessage) {
        ...
        // Detect turn complete (model done speaking)
